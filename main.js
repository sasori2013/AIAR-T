...
/**
 * Initialize 8thwall.org Image Target tracking events via XrController.configure
 */
const initAR = () => {
  ...
  const startXR = async () => {
    try {
      log('[OSS] Configuring XrController...');
      
      // Fetch the target JSON generated by image-target-cli
      const targetResponse = await fetch('./targets/logo-target.json');
      const targetData = await targetResponse.json();
      
      // Programmatically configure the controller with the target data
      ...
      log('[OSS] Listeners active and targets configured');
    } catch (err) {
      log(`[OSS] Init Error: ${err.message}`);
    }
  };
  ...
  if (window.XR8) {
    ...
    window.addEventListener('xr8enabled', startXR);
  }
};
